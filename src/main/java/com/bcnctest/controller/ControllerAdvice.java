...
public class ControllerAdvice {
    ...
    ResponseEntity<ErrorDTO> runtimeExceptionHandler(RuntimeException ex) {
        ErrorDTO errorDTO = ErrorDTO.builder().code("DB-500").message(ex.getMessage()).build();
    ...
    ResponseEntity<ErrorDTO> runtimeExceptionHandler(NoResourceFoundException ex) {
        ErrorDTO errorDTO = ErrorDTO.builder().code("AL-404").message(ex.getMessage()).build();
        return new ResponseEntity<>(errorDTO, HttpStatus.NOT_FOUND);
    }

    @ExceptionHandler(value = Exception.class)
    ResponseEntity<ErrorDTO> runtimeExceptionHandler(Exception ex) {
        ErrorDTO errorDTO = ErrorDTO.builder().code("AL-500").message(ex.getMessage()).build();
        return new ResponseEntity<>(errorDTO, HttpStatus.INTERNAL_SERVER_ERROR);
    }
}
